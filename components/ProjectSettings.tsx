...
import { ProjectSettings, DailyReport, QuantityEntry, ItemTypeDefinition } from '../types';
...
export const ProjectSettingsView: React.FC<ProjectSettingsProps> = ({ currentSettings, onSave, reports, quantities }) => {
  ...
  const [itemTypes, setItemTypes] = useState<ItemTypeDefinition[]>(currentSettings?.itemTypes || ITEM_PATTERNS.map(p => ({
      name: p.name,
      pattern: p.pattern.toString().slice(1, -2), // Remove slashes and 'i' flag for storage
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-20">
        <div className="flex justify-between items-center border-b border-slate-200 pb-4">
            <div>
                <h2 className="text-3xl font-bold text-slate-800">Project Configuration</h2>
                <p className="text-sm text-slate-500">Manage site hierarchy and custom work items.</p>
            </div>
