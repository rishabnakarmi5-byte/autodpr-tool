...
export const ActivityLogs: React.FC<ActivityLogsProps> = ({ logs, onRecover, onRestoreItem, onRestoreRaw, onRevertBulk }) => {
  ...
  const storageModal = isStorageOpen && (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-sm animate-fade-in text-left font-sans">
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in relative">
      ...
      {isStorageOpen && createPortal(storageModal, document.body)}
    </div>
  );
};
